...
// When the number of hits is 0 or 1 the rhythm is always 0x0 or 0x1 respectively,
// no need to store them in the look up table.
...
static uint32_t euclideanRhythm(unsigned int length, unsigned int on_beats)
{
        ...
        if(length <= 8)
        {
                ...
                // Index for this length starts at the sum of lengths up to this number,
                // then add the number of on beats.
